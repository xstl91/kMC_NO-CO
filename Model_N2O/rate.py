...
def _q_rot(mol):
    ...
    '''
    if mol == 'N2O':
        r_c = (M_O * Bond_l[mol]['NO'] - M_N * Bond_l[mol]['NN']) / Mass[mol]    # centered at central N
        r_N1 = - Bond_l[mol]['NN'] - r_c
        r_N2 = - r_c
        r_O = Bond_l[mol]['NO'] - r_c
        moment = M_N*r_N1**2 + M_N*r_N2**2 + M_O*r_O**2
        multi = 1
    else:
        if mol in ('NO', 'CO'):
